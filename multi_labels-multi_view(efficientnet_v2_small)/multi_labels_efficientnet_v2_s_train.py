    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_loss < best_loss:
                ...
                torch.save(model.state_dict(), 'C:/Users/mh661/Desktop/multi_labels-multi_view_10_classes/models/efficientnet_v2_s_cpk/efficientnet_v2_s-{}.ckpt'.format(epoch + 1))
...
if __name__ == "__main__":
    ...
    num_view = 12
    train_dataset_m = MultiviewImgDataset(root_dir='C:/Users/mh661/Desktop/modelnet10_reduce/*/train', scale_aug=False,
    ...
    val_dataset_m = MultiviewImgDataset(root_dir='C:/Users/mh661/Desktop/modelnet10_reduce/*/test', scale_aug=False,
                                        rot_aug=False, num_views=12)
    ...
    model_ft.features[0] = nn.Conv2d(num_view, 24, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
    num_ftrs = model_ft.classifier[1].in_features
    model_ft.classifier[1] = nn.Linear(num_ftrs, number_of_tags)
    model = nn.Sequential(model_ft, nn.Sigmoid()).to(device)

    #model.load_state_dict(torch.load("C:/Users/mh661/Desktop/multi_labels-multi_view_10_classes/models/efficientnet_v2_s_cpk/efficientnet_v2_s-6.ckpt"))  # 0.7
    for param in model[0].parameters():
        param.requires_grad = False
    # for param in model[0].features[2].parameters():
    #     param.requires_grad = True
    # for param in model[0].features[3].parameters():
    #     param.requires_grad = True
    # for param in model[0].features[4].parameters():
    #     param.requires_grad = True
    for param in model[0].features[5].parameters():
        param.requires_grad = True
    for param in model[0].features[6].parameters():
        param.requires_grad = True
    for param in model[0].features[7].parameters():
        param.requires_grad = True
    ...
    # for param in model[0].parameters():
    #     param.requires_grad = False
    # for param in model[0].features[0].parameters():
    #     param.requires_grad = True
    # # for param in model[0].features[1].parameters():
    # #     param.requires_grad = True
    # # for param in model[0].features[3].parameters():
    # #     param.requires_grad = True
    # for param in model[0].classifier.parameters():
    ...
    # rand_idx = np.random.permutation(int(len(train_loader_m.dataset.filepaths) / num_views))
    # filepaths_new = []
    # for i in range(len(rand_idx)):
    #      filepaths_new.extend(train_loader_m.dataset.filepaths[
    #                          rand_idx[i] * num_views:(rand_idx[i] + 1) * num_views])
    # train_loader_m.dataset.filepaths = filepaths_new
    #
    # for data in tqdm(train_loader_m):
    #     inputs, labels = data['image'].to(device), data['labels'].type(
    #         torch.FloatTensor).to(device)
    #     aaa=np.array(inputs)
    #     bbb = np.array(labels)
    #     print('labels:',labels)
    #     #print('labels:', train_loader_m.dataset.filepaths)
    # plt.imshow(aaa[10][1])

    criterion = nn.BCELoss()
    optimizer_ft = optim.Adam(model.parameters(), lr=0.0005 ,weight_decay=0 ,betas=(0.9, 0.999))
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=30, gamma=0.1)

    train_model(model, train_loader_m, val_loader_m, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=6)
